...
const PatientForm: React.FC = () => {
  ...
  useEffect(() => {
    if (typeof crypto !== 'undefined' && 'randomUUID' in crypto) {
      ...
      setSessionId(`session-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    }
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 border-b pb-4 gap-2">
        <h2 className="text-2xl font-bold text-gray-800">ข้อมูลผู้ป่วย (Patient Information)</h2>
        ...
        <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-md border border-gray-100">
          ...
          <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
            syncStatus === 'submitted' ? 'bg-green-100 text-green-700' :
            syncStatus === 'actively filling in' ? 'bg-blue-100 text-blue-700 animate-pulse' :
            'bg-gray-200 text-gray-600'
          }`}>
            {syncStatus === 'submitted' ? 'ส่งข้อมูลแล้ว' :
             syncStatus === 'actively filling in' ? 'กำลังพิมพ์...' : 
             'พักหน้าจอ'}
          </span>
        </div>
      </div>
